...
let carts=[];
let m = "All"
const b = products.reduce((pre,value)=>{
...
const buttons = b.map(function (value) {
...
function display(a) {
  const myproducts = a.map(function (value,index) {
...
let AddtoCart =(index)=>{
  let selectedProductsitem = products[index];

  let exists = carts.find((value)=>{
    return value.id==selectedProductsitem.id
  })
  if(exists==undefined){
    selectedProductsitem.qty=1;
    carts.push(selectedProductsitem)
  }
  else{
    selectedProductsitem.qty=selectedProductsitem.qty+1
  }
  document.getElementById("count").innerHTML=carts.length
  // document.getElementById("qtyy").innerHTML=selectedProductsitem.qty
  console.log(selectedProductsitem.qty)
}